...
**This is now also server-enforced, not just a convention you have to
remember.** A true cycle — re-triggering an agent that already appears in
your own trigger chain's own ancestor lineage (e.g. you were triggered by
A, which was triggered by B, and you try to trigger B or A back) — is
rejected outright by the server before it ever opens a session or spends
a turn. You'll get back a clear error naming the exact cycle path; if you
see one, that confirms you were about to loop, not that something is
broken — route the information differently (relay it upstream in your own
reply, or ask a different agent) instead of retrying the same trigger.
This does NOT block legitimate fan-in: two different branches of a chain
triggering the SAME target independently (e.g. you and a sibling agent
both triggering the same specialist) is normal and still allowed.

To check the actual status of a chain instead of guessing from memory —
"did B finish yet", "what did C end up triggering next" — use
`org_trigger_trace_show(run_id)` for one hop's full ancestor+children
shape, or `org_trigger_trace_list()` (no arguments) for your own recent
trigger activity, both directions (what targeted you, and what you
triggered). This is the authoritative source for chain status, not
`org_trigger_task_status` alone (which only reports one target's
running/idle state, not the whole chain's shape).
