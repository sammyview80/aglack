...
1. Finish `DockerCliLauncher`: port publishing, named volumes, boot-script
   ...
   check before returning `ready`.
2. Wire the new `revamp/frontend`'s `CreateWorkspace.tsx` form submission
   to actually POST to `rust_gateway`'s `/workspaces` endpoint (currently
   it's rendered but not confirmed wired to a real submit handler calling
   this new backend — check `CreateWorkspace.tsx`'s `onCreate` prop usage
   in `CreatePage.tsx` before assuming this is already connected).
